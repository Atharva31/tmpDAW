...
def clean_acs_data(df):
    ...
    # Critical columns must exist and be non-null
    critical_cols = ["ZIP", "population", "median_income"]
    df = df.dropna(subset=critical_cols)

    # Drop ZCTAs with unrealistically small populations (likely PO Boxes)
    df = df[df["population"] >= 100]

    # Drop non-positive income values
...
def merge_eia_acs(eia_df, acs_df):
    ...
    print(f"[Merge] Loss: {loss_pct:.1f}% of EIA ZIPs (PO Box-only, non-ZCTA, etc.)")
...
def filter_nyc_la(df):
    ...
    for start, end in nyc_ranges:
        df_filtered.loc[
            (df_filtered["ZIP_int"] >= start) & (df_filtered["ZIP_int"] <= end), "city"
        ] = "NYC"
